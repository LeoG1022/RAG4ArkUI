...
18. **概念问答必须询问归档（FAIL级硬性规则）**：
    - **触发条件**：用户问以下任一意图的概念解释类问题
      - 「X 是什么」「X 有什么用」「X 有什么功能」
      - 「为什么用 X」「X 跟 Y 啥区别」
      - 「X 怎么工作」「X 的原理」
    - **强制动作**：Agent 解答完毕后，**必须**主动询问用户：
      > 「这个解答要不要归档到 `docs/concepts/<term>.md`？」
    - **禁止**：默认归 / 默认不归 / 沉默掠过 / 把决策权留给自己
    - **用户确认归档后**：按 `docs/concepts/README.md` 「加新条目的步骤」5 步执行（写 concepts/ + 更 README + GLOSSARY 加链 + mdbook include + SUMMARY 加项）
    - **用户拒绝归档**：本轮结束，下次同主题再问时仍需重复询问
    - **机械化校验**：无（属对话行为规则，无法静态扫描）；违反时由用户事后指出
    - **理由**：概念问答是高复用知识资产，agent 自身无法判断是否值得沉淀，必须把决策权交还用户；同时强制 agent 在每次概念问答后停下来评估"本轮对话是否产生了知识增量"。详见 `docs/concepts/README.md`「Agent 自我约束」节

<!-- [可选业务规则示例] 以下规则 #19 是业务特定硬约束的格式示例。
     使用者按自己的项目需求添加类似规则（例如：特定目录的代码必须通过编译验证才能提交）。
     如无对应业务需求，可删除此注释块。
19. **[项目特定] 业务代码改动必须通过编译验证（FAIL级硬性规则示例）**：
