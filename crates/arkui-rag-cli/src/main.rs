...
async fn serve_lsp_impl(
    ...
    use arkui_rag_server::{serve_lsp_stdio, AppState};
...
async fn serve_lsp_impl(
    ...
    anyhow::bail!(
        "本二进制未启用 lsp feature。重新构建：\n\
         \tcargo build -p arkui-rag-cli --features lsp --release\n\
         （或 --features full 启用全套）"
    )
}
